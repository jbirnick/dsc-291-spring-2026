...
lemma perceptron_lemma1 (z : ℕ → E) (w_star : E) (gamma : ℝ) (M : ℕ)
  ...
  | succ M ih =>
    ...
    have h1 : (M : ℝ) * gamma ≤ ⟪w_star, perceptronWeights z M⟫_ℝ :=
      ih (fun i hi => hgamma i (by omega))
...
lemma perceptron_lemma2 (z : ℕ → E) (R : ℝ) (M : ℕ)
  ...
  | succ M ih =>
    ...
    have h1 : ‖perceptronWeights z M‖ ^ 2 ≤ (M : ℝ) * R ^ 2 :=
      ih (fun i hi => hmistake i (by omega)) (fun i hi => hbound i (by omega))
...
theorem perceptron_mistake_bound (z : ℕ → E) (w_star : E) (gamma R : ℝ) (M : ℕ)
  ...
  have hcs : ⟪w_star, perceptronWeights z M⟫_ℝ ≤ ‖perceptronWeights z M‖ := by
    calc
      ⟪w_star, perceptronWeights z M⟫_ℝ ≤ ‖w_star‖ * ‖perceptronWeights z M‖ :=
        real_inner_le_norm w_star (perceptronWeights z M)
  ...
  · have hMpos_real : 0 < (M : ℝ) := hMpos
    ...
    have H : ((M : ℝ) ≤ R ^ 2 / gamma ^ 2) ↔ ((M : ℝ) * gamma ^ 2 ≤ R ^ 2) :=
      le_div_iff₀ hgamma2_pos
    exact H.mpr hl8
